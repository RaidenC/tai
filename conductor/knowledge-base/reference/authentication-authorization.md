...
- **What:** DPoP (RFC 9449) upgrades standard Bearer Tokens to "Sender-Constrained" tokens by cryptographically binding the token to the client's device-generated key pair. Even if a token is stolen via XSS or network interception, it is useless without the client's non-extractable private key.
- **Why:** Completely neutralizes token theft — the most critical vulnerability in traditional JWT architectures. Becoming mandatory for FAPI 2.0 (banking), healthcare, and AI Agent workflows (Model Context Protocol).
- **How:** The Angular frontend generates an ECDSA P-256 key pair via Web Crypto API, sends the public key during login, and signs every API request with a short-lived DPoP Proof. The API validates both the Access Token and the proof. See **[[Security-CSP-DPoP]]** for the full 4-step mechanism, implementation details, and trade-offs.
- **Trade-offs:** Adds ~1-3ms cryptographic overhead per request on the frontend, and the backend must validate two JWTs per request. Requires careful clock synchronization (NTP) between client and server.
