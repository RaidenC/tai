...
## Phase 3: Global Authentication Rollout
**Goal: Iteratively migrate existing tests to the global auth state.**
1. **Migrate by Domain/Folder:** Select one group of tests at a time (e.g., just the `users/` tests or just the `privileges/` tests).
2. **Remove Redundant Logins:** Remove their `beforeEach` UI login logic and configure them to use the global `storageState`.
3. **Verification:** Run the E2E suite after each group is migrated to catch any state-leakage or dependency issues. Repeat until all tests are migrated.

## Phase 4: TDM API Setup Utility & Pilot
**Goal: Create reusable API seed data helpers without breaking UI setup.**
1. **Create TDM Utility:** Create a `test-data.ts` helper file that uses Playwright's `APIRequestContext` to call `POST /api/tdm/seed-user`.
2. **Pilot Test Refactor:** Pick a single, non-critical test that currently uses the UI to create a user before editing them. Refactor it to use the new TDM utility instead.
3. **Verification:** Run the specific test to ensure the API correctly seeds the data and the test passes.

## Phase 5: TDM API Setup Rollout
**Goal: Replace slow UI data creation steps across the suite.**
1. **Iterative Refactor:** Go through the test suite, one spec file at a time, replacing UI-based "Arrange" steps with TDM API calls.
2. **Verification:** Run the E2E suite after modifying each spec file. Do not merge large batches of these changes at once.

## Phase 6: Auto-Waiting & Mock Networks (Targeted Fixes)
**Goal: Replace brittle `waitForTimeout` calls gradually.**
1. **Code Audit & Search:** Search the codebase for `page.waitForTimeout()`.
2. **Iterative Replacement:** Replace hardcoded waits in one spec file at a time with strict `expect().toBeVisible()` or `expect().toHaveText()` assertions.
3. **Network Mocks (As Needed):** Where UI behavior relies on slow or flaky third-party endpoints, introduce `page.route` to mock the response for that specific test.
4. **Verification:** Run the modified test locally using `--repeat-each=5` to ensure the flakiness is actually resolved before committing.
