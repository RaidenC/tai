...
- Add Storybook stories for default, completed, current, blocked, error, mobile, and security-relevant states.
...
- Deprecate or replace `WizardComponent` with a compatibility path only if existing imports require a transition.
...
The stepper receives a list of steps and a selected step id. It renders progress, labels, and state indicators. It emits a `stepSelected` event when a selectable step is activated. It does not know what a route is and does not render `routerLink`.
...
- NgRx selectors and route guards determine whether steps are complete, current, blocked, or error.
...
- `current`: selected step with strong focus/progress treatment.
...
- Completed steps include accessible text that communicates completion without relying on color alone.
- Buttons have visible focus states with sufficient contrast.
- Touch targets should be at least 44px in comfortable density.
- Keyboard interaction must work with `Tab`, `Enter`, and `Space`.
...
- Map claim validity selectors into `completed`, `current`, `blocked`, or `error` step statuses.
...
`claimStepGuard` remains the route access source of truth. The stepper can show blocked state, but it must not be the security or workflow enforcement mechanism.

## Compatibility Strategy

Prefer replacing `WizardComponent` with `StepperComponent` directly because `WizardComponent` currently has a misleading architecture role.

If a transition is needed, keep a short-lived compatibility `WizardComponent` wrapper that:

- Lives outside the generic `stepper` folder.
- Is marked deprecated in code comments.
- Imports `StepperComponent`.
- Keeps router-aware behavior only for existing consumers.
- Is removed once borrower portal consumers migrate.

The preferred implementation path is direct migration because current search results show the borrower claim wrapper as the primary consumer.
...
5. Remove or deprecate `WizardComponent` after verifying no remaining consumers.
...
- Public exports no longer require consumers to import a router-aware wizard for step progress UI.
...
- The design-system component will be called `StepperComponent`, not `WizardComponent`.
